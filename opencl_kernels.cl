...
//shared 16*16 kernel méret opt. 2 kimenet egyszerre
...
//Shared 32*16 2 kimenet, leggyorsabb 2021.11.10
...
//Shared 32*8 kernel 4 kimenet le kell negyedelni az indított y szálakat
__kernel void median_filter_BMS2_shared_32x8(__global unsigned char* gInput,
	__global unsigned char* gOutput,
	__constant float* filter_coeffs,
	int imgWidth,
	int imgWidthF)
{
	__local int sh_mem[36][36][3];
	int th_load_id = get_local_id(1) * get_local_size(0) + get_local_id(0);
	int start_id = 3 * 4 * (get_group_id(1) * get_local_size(1) * imgWidthF) + 3 * (get_group_id(0) * get_local_size(0));

	for (int loader = 0; loader < 18; loader++)
	{
		if (th_load_id < 216)
		{
			sh_mem[th_load_id / 108 + loader * 2][(th_load_id / 3) % 36][th_load_id % 3] = gInput[start_id + 2 * loader * 3 * imgWidthF + (th_load_id / 108) * 3 * imgWidthF + th_load_id % 108];
		};
	};
	barrier(CLK_LOCAL_MEM_FENCE);
	...
	int X = get_local_id(0);
	int Y = 4 * get_local_id(1);

	int PX = get_group_id(0) * get_local_size(0) + get_local_id(0);
	int PY = 4 * (get_group_id(1) * get_local_size(1) + get_local_id(1));


#pragma unroll
	for (int col_control = 0; col_control < 2; col_control++)
	{
#pragma unroll
		for (int rgb = 0; rgb < 3; rgb++)
		{
			int loaded_value0 = sh_mem[Y + col_control * 2][X][rgb];
			int loaded_value1 = sh_mem[Y + col_control * 2][X + 1][rgb];
			int loaded_value2 = sh_mem[Y + col_control * 2][X + 2][rgb];
			int loaded_value3 = sh_mem[Y + col_control * 2][X + 3][rgb];
			int loaded_value4 = sh_mem[Y + col_control * 2][X + 4][rgb];

			int loaded_value5 = sh_mem[Y + 1 + col_control * 2][X][rgb];
			int loaded_value6 = sh_mem[Y + 1 + col_control * 2][X + 1][rgb];
			int loaded_value7 = sh_mem[Y + 1 + col_control * 2][X + 2][rgb];
			int loaded_value8 = sh_mem[Y + 1 + col_control * 2][X + 3][rgb];
			int loaded_value9 = sh_mem[Y + 1 + col_control * 2][X + 4][rgb];

			int loaded_value10 = sh_mem[Y + 2 + col_control * 2][X][rgb];
			int loaded_value11 = sh_mem[Y + 2 + col_control * 2][X + 1][rgb];
			int loaded_value12 = sh_mem[Y + 2 + col_control * 2][X + 2][rgb];
			int loaded_value13 = sh_mem[Y + 2 + col_control * 2][X + 3][rgb];
			int loaded_value14 = sh_mem[Y + 2 + col_control * 2][X + 4][rgb];

			int loaded_value15 = sh_mem[Y + 3 + col_control * 2][X][rgb];
			int loaded_value16 = sh_mem[Y + 3 + col_control * 2][X + 1][rgb];
			int loaded_value17 = sh_mem[Y + 3 + col_control * 2][X + 2][rgb];
			int loaded_value18 = sh_mem[Y + 3 + col_control * 2][X + 3][rgb];
			int loaded_value19 = sh_mem[Y + 3 + col_control * 2][X + 4][rgb];

			int loaded_value20 = sh_mem[Y + 4 + col_control * 2][X][rgb];
			int loaded_value21 = sh_mem[Y + 4 + col_control * 2][X + 1][rgb];
			int loaded_value22 = sh_mem[Y + 4 + col_control * 2][X + 2][rgb];
			int loaded_value23 = sh_mem[Y + 4 + col_control * 2][X + 3][rgb];
			int loaded_value24 = sh_mem[Y + 4 + col_control * 2][X + 4][rgb];

			int loaded_value25 = sh_mem[Y + 5 + col_control * 2][X][rgb];
			int loaded_value26 = sh_mem[Y + 5 + col_control * 2][X + 1][rgb];
			int loaded_value27 = sh_mem[Y + 5 + col_control * 2][X + 2][rgb];
			int loaded_value28 = sh_mem[Y + 5 + col_control * 2][X + 3][rgb];
			int loaded_value29 = sh_mem[Y + 5 + col_control * 2][X + 4][rgb];
			...
			gOutput[3 * ((2 * col_control + PY) * imgWidth + PX) + rgb] = kernel1_5;
			//
			gOutput[3 * ((2 * col_control + PY + 1) * imgWidth + PX) + rgb] = kernel2_5;
